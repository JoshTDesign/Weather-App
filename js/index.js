...
var uvColor;
var APIKey = "b8e4a5fc7efffbddda3bb2a508702e23"

//checks the past search array and fills the history list
function initPage(){
    $('#history').children().remove();  //clear history list  
    var tempArray = JSON.parse(localStorage.getItem('searchedArray'));  //Put saved history list to temp array
    if (tempArray != null) {
      searchedArray = tempArray;  // If there was a search history assign it to 'searchedArray'
    }

    searchedArray.reverse();  // reverse order for search history so that it orders from newest to oldest
    for (var i = 0; i < searchedArray.length; i++) {  // Loop through all history items
        ...
        $('#history').append(cityLi); // Append a new element for each history item
    }
    searchedArray.reverse();  // retrn searched Array to normal order
};
